...
open class DiscoverProcessWorker {

    private var client: CommandClient? = null

    @Volatile
    private var detectedDeviceID: String? = null

    @Volatile
    internal var isDetectedDeviceClaimed: Boolean = false
    @Volatile
    internal var gotOwnershipInfo: Boolean = false
    @Volatile
    internal var needToClaimDevice: Boolean = false
    ...
    internal fun withClient(client: CommandClient): DiscoverProcessWorker {
        this.client = client
        return this
    }
    ...
    fun doTheThing() {
        ...
        if (!truthy(detectedDeviceID)) {
            try {
                val response = client?.sendCommand(DeviceIdCommand(), DeviceIdCommand.Response::class.java)
                ...
                e.printStackTrace()
                throw SetupStepException("Process died while trying to get the device ID", e)
            }
        ...
        if (DeviceSetupState.publicKey == null) {
            try {
                DeviceSetupState.publicKey = publicKey
            } catch (e: Crypto.CryptoException) {
                throw SetupStepException("Unable to get public key: ", e)

            } catch (e: IOException) {
                throw SetupStepException("Error while fetching public key: ", e)
            }
        ...
        if (!gotOwnershipInfo) {
            ...
            if (!isDetectedDeviceClaimed) {
                ...
                var deviceClaimedByUser = false
                for (deviceId in DeviceSetupState.claimedDeviceIds) {
                    if (deviceId.equals(detectedDeviceID!!, ignoreCase = true)) {
                ...
                if (isDetectedDeviceClaimed && !deviceClaimedByUser) {
                    ...
                    throw DiscoverDeviceFragment.DeviceAlreadyClaimed("Device already claimed by another user")
    ...
    companion object {
        private val log = TLog.get(DiscoverProcessWorker::class.java)
    }
}
